...
export default function PostEditor({ post }: Props) {
  ...
  const handleTitleChange = useCallback((val: string) => {
    setTitle(val);
    if (!isEdit || !slug) {
      // 纯 ASCII slug：时间戳格式，避免中文 URL 兼容问题
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-10 animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-[28px] font-semibold tracking-[-0.02em] text-apple-text dark:text-dark-text">{isEdit ? '编辑文章' : '写新文章'}</h1>
      ...
      <form className="space-y-6 card p-6 sm:p-8">
        <div>
          <label className="block text-xs font-medium text-apple-secondary dark:text-dark-secondary mb-2">标题</label>
        ...
        <div>
          <label className="block text-xs font-medium text-apple-secondary dark:text-dark-secondary mb-2">链接</label>
        ...
        <div>
          <label className="block text-xs font-medium text-apple-secondary dark:text-dark-secondary mb-2">摘要</label>
          <input type="text" value={excerpt} onChange={(e) => setExcerpt(e.target.value)} className="input-apple" placeholder="简短描述这篇文章..." />
        </div>
        <div>
          <label className="block text-xs font-medium text-apple-secondary dark:text-dark-secondary mb-2">封面图片</label>
          <div className="flex items-center gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-apple-secondary dark:text-dark-secondary mb-2">标签（用逗号分隔）</label>
